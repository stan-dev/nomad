...
TEST(ScalarSmoothFunctions, Pow) {
  ...
  nomad::tests::test_derivatives<true, true, pow_vv_func>(x1);
  
  Eigen::VectorXd x2 = Eigen::VectorXd::Ones(1);
  x2 *= 0.576;
  
  nomad::tests::test_derivatives<true, true, pow_vd_func>(x2);
  nomad::tests::test_derivatives<true, true, pow_dv_func>(x2);
}
