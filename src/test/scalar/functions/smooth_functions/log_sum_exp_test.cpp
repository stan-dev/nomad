...
template <typename T>
class log_sum_exp_vv_func: public nomad::base_functor<T> {
public:
...
template <typename T>
class log_sum_exp_vd_func: public nomad::base_functor<T> {
public:
  T operator()(const Eigen::VectorXd& x) const {
    T v = x[0];
...
template <typename T>
class log_sum_exp_dv_func: public nomad::base_functor<T> {
public:
  T operator()(const Eigen::VectorXd& x) const {
    T v = x[0];
...
TEST(ScalarSmoothFunctions, LogSumExp) {
  ...
  nomad::tests::test_function<true, log_sum_exp_vv_func>(x1);
  ...
  nomad::tests::test_function<true, log_sum_exp_vd_func>(x2);
  nomad::tests::test_function<true, log_sum_exp_dv_func>(x2);
