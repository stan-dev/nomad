...
TEST(ScalarNonSmoothFunctions, Fdim) {
  ...
  nomad::tests::test_derivatives<false, false, fdim_vv_func>(x1);
  
  x1 *= -1;
  nomad::tests::test_derivatives<false, false, fdim_vv_func>(x1);
  
  Eigen::VectorXd x2 = Eigen::VectorXd::Ones(1);
  nomad::tests::test_derivatives<false, false, fdim_vd_func>(x2);
  
  x2[0] = 0.5;
  nomad::tests::test_derivatives<false, false, fdim_dv_func>(x2);
