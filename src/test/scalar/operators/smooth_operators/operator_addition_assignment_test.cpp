...
TEST(ScalarSmoothOperators, OperatorAdditionAssignment) {
  ...
  nomad::tests::test_derivatives<true, true, operator_addition_assignment_vv_func>(x1);
  
  Eigen::VectorXd x2 = Eigen::VectorXd::Ones(1);
  x2 *= 0.576;
  
  nomad::tests::test_derivatives<true, true, operator_addition_assignment_vd_func>(x2);
}
