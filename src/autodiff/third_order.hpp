...
namespace nomad {
  ...
  void third_order_forward_val(const T_var& v) {
    for (nomad_idx_t i = 1; i <= v.node(); ++i)
      var_nodes::address[i].third_order_forward_val();
  }
  
  template<class T_var>
  void third_order_reverse_adj(const T_var& v) {
    var_nodes::address[v.node()].third_grad() = 0;
    var_nodes::address[v.node()].fourth_grad() = 0;
    for (nomad_idx_t i = v.node(); i > 0; --i)
      var_nodes::address[i].third_order_reverse_adj();
  }
  ...
  grad_hessian(const F& functional,
    ...
    try {
      ...
      for (eigen_idx_t i = 0; i < d; ++i) {
        ...
        v(j) = var_nodes::address[j + 1].second_grad();
        
        H.col(i) = v;
        
        // Third-order
        for (eigen_idx_t j = 0; j < d; ++j)
        var_nodes::address[j + 1].fourth_val() = 0;
        
        for (eigen_idx_t k = 0; k <= i; ++k) {
          
          for (eigen_idx_t j = 0; j < d; ++j)
          var_nodes::address[j + 1].third_val() = static_cast<double>(k == j);
          
          third_order_forward_val(f_var);
          
          for (eigen_idx_t j = 0; j < d; ++j)
          v(j) = var_nodes::address[j + 1].fourth_grad();
  ...
  grad_trace_matrix_times_hessian(const F& functional,
    ...
    try {
      ...
      for (eigen_idx_t i = 0; i < d; ++i) {
        ...
        for (eigen_idx_t j = 0; j < d; ++j)
        var_nodes::address[j + 1].third_val() = M(j, i);
